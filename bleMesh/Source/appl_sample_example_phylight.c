...
//    UCHAR  * t_data;
  ...
  switch (event_type) {
    ...
    case PROV_EVT_OOB_DISPLAY: CONSOLE_OUT("Recvd PROV_EVT_OOB_DISPLAY\n");
      CONSOLE_OUT("Status - 0x%04X\n", event_result);
    ...
    case PROV_EVT_OOB_ENTRY: CONSOLE_OUT("Recvd PROV_EVT_OOB_ENTRY\n");
      CONSOLE_OUT("Status - 0x%04X\n", event_result);
      /* Reference the Authenticatio Type information */
      oob_info = (PROV_OOB_TYPE_S *) event_data;
      CONSOLE_OUT("Authenticaion Action - 0x%02X\n", oob_info->action);
    ...
    case PROV_EVT_DEVINPUT_COMPLETE: CONSOLE_OUT("Recvd PROV_EVT_DEVINPUT_COMPLETE\n");
      CONSOLE_OUT("Status - 0x%04X\n", event_result);
      break;

    case PROV_EVT_PROVDATA_INFO: CONSOLE_OUT("Recvd PROV_EVT_PROVDATA_INFO\n");
      CONSOLE_OUT("Status - 0x%04X\n", event_result);
    ...
    case PROV_EVT_PROVISIONING_COMPLETE: CONSOLE_OUT("Recvd PROV_EVT_PROVISIONING_COMPLETE\n");
      CONSOLE_OUT("Status - 0x%04X\n", event_result);
...
void UI_proxy_callback
  ...
  switch (p_evt) {
    case MS_PROXY_UP_EVENT: CONSOLE_OUT(
    ...
    case MS_PROXY_DOWN_EVENT: CONSOLE_OUT(
...
void UI_gatt_iface_event_pl_cb
  ...
  switch (ev_name) {
    ...
    case BLEBRR_GATT_IFACE_ENABLE: CONSOLE_OUT("\r\n >> GATT Bearer Active Event Received!\r\n");
      {
    ...
    case BLEBRR_GATT_IFACE_DISABLE: CONSOLE_OUT("\r\n >> GATT Bearer Inactive Event Received!\r\n");
      break;
...
API_RESULT UI_app_config_server_callback(
  ...
  switch (opcode) {
    case MS_ACCESS_CONFIG_NODE_RESET_OPCODE: CONSOLE_OUT("[ST TimeOut CB]\n");
      proxy_state = UI_proxy_state_get();
...
API_RESULT UI_sample_get_net_key(void) {
  ...
  if (API_SUCCESS == retval) {
    CONSOLE_OUT(
        "Network Key[0x%02X]: %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X\n",
