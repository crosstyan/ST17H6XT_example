...
static struct SimpleGattScanServer SimpleClientInfo;
static struct SimpleClientADV_ScanData simpleBLEDevList[DEFAULT_MAX_SCAN_RES];
...
//    static int try_num = 0;
  switch (pEvent->gap.opcode) {
    ...
    case GAP_LINK_TERMINATED_EVENT: {
      ...
      AT_LOG("[PMCNT] rdErr %d rstErr %d trgErr %d\n", g_pmCounters.ll_rfifo_read_err, g_pmCounters.ll_rfifo_rst_err,
             g_pmCounters.ll_trigger_err);
      // Terminate Link , memset SimpleGattScanServer structure
      osal_memset(&SimpleClientInfo, 0, sizeof(struct SimpleGattScanServer));
  ...
  if (simpleBLEDiscState == BLE_DISC_STATE_SVC) {
    ...
    if (pMsg->method == ATT_READ_BY_TYPE_RSP &&
      ...
      for (unsigned char i = 0;
           i < SimpleClientInfo.ServerGroupService[SimpleClientInfo.PrimaryCharacIndex].CharacNum; i++) {
...
//              LOG("Should Update Properties Value \r\n");
  ...
  LOG("Service UUID : 0x%02X%02X, Characteristic UUID:0x%04X ,PrimaryServiceCnt %d,PrimaryCharacIndex %d,Properties 0x%02X\r\n", \
        ...
        SimpleClientInfo.ServerGroupService[PrimaryServiceCnt].Characteristic[PrimaryCharacIndex].charUuid,
      PrimaryServiceCnt, PrimaryCharacIndex, Properties);
  ...
  if ((Properties & GATT_PROP_BCAST) || \
    ...
    pReqread = reinterpret_cast<attReadReq_t *>(osal_mem_alloc(sizeof(attReadReq_t)));
